...
const SettingsScreen = () => {
  ...
  return (
    <Container className="mobile-container p-4 pb-5">
      ...
      {/* Profile Header */}
      <div 
        className="d-flex align-items-center gap-3 p-4 mb-4 bg-ios-secondary rounded-4 cursor-pointer"
        onClick={() => navigate('/profile')}
      >
        <UserAvatar name={userFullLabel} size={60} />
        <div className="flex-grow-1 overflow-hidden">
          <h2 className="h4 fw-bold mb-0 text-white text-truncate">{userFullLabel}</h2>
          <p className="small text-ios-gray mb-0 text-truncate">{profile?.email}</p>
        </div>
      ...
      <Card className="bg-ios-dark-gray border-0 overflow-hidden mb-4 shadow-none rounded-4">
        <ListGroup variant="flush" className="bg-transparent">
          ...
          >
            ...
            <ExternalLink size={16} className="text-ios-gray opacity-40" />
          </ListGroup.Item>
        </ListGroup>
      </Card>
